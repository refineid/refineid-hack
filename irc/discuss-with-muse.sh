...
case "$COMMAND" in
  start)
    ...
    notify_irc antigravity "#code-review" "--- Review Discussion Started for $REPO PR #$PR_NUM (Session: $SESSION_ID) ---"
    if [[ -n "$INTENT" ]]; then
      notify_irc antigravity "#code-review" "Author intent: $INTENT"
    fi
...
EOF
    ...
    "$MUSE_BIN" exec --yolo --session-id "$SESSION_ID" --prompt-file "$TMP_DIR/prompt.md" > "$TMP_DIR/muse_reply.txt" 2>&1 || {
...
EOF
    ...
    notify_irc muse "#code-review" "$(cat "$TMP_DIR/cleaned_reply.txt")"
  ...
  reply)
    ...
    notify_irc antigravity "#code-review" "[Turn $NEXT_TURN to Muse]: $CONTENT"
...
EOF
    ...
    "$MUSE_BIN" exec --yolo --session-id "$SESSION_ID" --prompt-file "$TMP_DIR/reply_prompt.md" > "$TMP_DIR/muse_reply.txt" 2>&1 || {
      echo "Error: muse exec failed" >&2
      cat "$TMP_DIR/muse_reply.txt" >&2
      exit 1
    }
...
EOF
    ...
    notify_irc muse "#code-review" "[Turn $NEXT_TURN from Muse]: $(cat "$TMP_DIR/cleaned_reply.txt")"
  ...
  submit)
    ...
    notify_irc antigravity "#code-review" "Discussion transcript posted to GitHub PR #$PR_NUM ($SUBMIT_EVENT)."
