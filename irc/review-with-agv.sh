...
# review-with-agv.sh - Run automated code review using Antigravity (agy / agv) agent

PROGNAME="review-with-agv"
AGY_BIN="${AGY_BIN:-/Users/pk/.local/bin/agy}"

print_usage() {
  echo "Usage: $PROGNAME [OPTIONS] [GIT_REF_OR_RANGE]"
  echo ""
  echo "Run an automated code review using Antigravity agent (agy --print)"
  echo "against a GitHub PR, a git commit range, or the current working tree diff."
  echo ""
  echo "Options:"
...
TMP_DIR="$(mktemp -d -t review-with-agv-XXXXXX)"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
echo "==> Invoking Antigravity reviewer ($AGY_BIN --dangerously-skip-permissions --print)..." >&2

"$AGY_BIN" --dangerously-skip-permissions --print "$(cat "$PROMPT_FILE")" > "$REVIEW_FILE" 2>&1 || {
